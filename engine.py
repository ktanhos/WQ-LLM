...
class GeneratorEngine:
    ...
    def _generate_pairwise(self) -> Iterable[str]:
        ...
        for field_a, field_b in pairs:
            for template in templates:
                if len(results) >= self.request.limit:
                    return results
    ...
    def _generate_mutate(self) -> Iterable[str]:
        ...
        results: List[str] = []
        guard = 0
        max_guard = self.request.limit * 60 + 1000
        while len(results) < self.request.limit and guard < max_guard:
            guard += 1
    ...
    def _finalize(self, expressions: Iterable[str]) -> List[str]:
        """Khử trùng lặp, giữ nguyên thứ tự và loại biểu thức quá dài."""
        seen = set()
        output: List[str] = []
